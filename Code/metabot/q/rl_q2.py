...
class rl():  # sarsa lambda
	...
	def make_policy(self):
		...
		"""
		#print("state:", self.state_prev)
		softmax_predictions = self.softmax(self.q[self.state_prev, :])
		len(softmax_predictions)
		#print("Q_max: ",max(self.q[self.state_prev, :]))
		#print("Q_min: ",min(self.q[self.state_prev, :]))
	...
	def make_policy_aft(self):
		...
		Returns: action to pick, note that the return value is the index of the action in action_space
		"""
		#print("state:", self.state_aft)
		softmax_predictions = self.softmax(self.q[self.state_aft, :])
	...
	def q_learning(self, next_state, reward):
		...
		mask = reward + self.discount_factor * self.q[self.state_aft, self.action_aft_policy] - self.q[
	...
	def q_learning1(self, next_state, reward):
		# best_action = make_policy(self.q[state,:])
		# excute action, get reward and the next state
		self.state_aft = next_state
		self.make_policy_aft()
		...
		mask = reward + self.discount_factor * self.q[self.state_aft, self.action_aft_policy] - self.q[
		self.state_prev, self.action_prev]
		self.q[self.state_prev, self.action_prev] += self.alpha * mask
		self.state_prev = self.state_aft
		self.action_prev = self.action_aft_policy
